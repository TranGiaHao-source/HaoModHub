...
icon.Image = "rbxassetid://116777271888123"
...
desc.Text = "Auto bonds beta version is running...)"
...
if game.PlaceId == 70876832253163 or game.PlaceId == 98018823628597 then
    Notif.New("[HaoModHub v1.0] Auto Bond Loaded!", 5)
    ...
    local function resetCharacter()
        if humanoid and humanoid.Health > 0 then
            print("[NightX Hub] Resetting character...")
            humanoid.Health = 0 -- Kills character to trigger respawn
        else
            warn("[NightX Hub] Cannot reset: Character already dead or invalid!")
        end
    end
        ...
        if humanoid.Sit then
            ...
            if seatCount == 1 then
                ...
                while true do
                    ...
                    if not bond then
                        warn("[Skull Hub] Detected vampire castle, tp to it...")
    ...
    local function collectBonds()
        ...
        if not character or not character:FindFirstChild("HumanoidRootPart") then
            warn("[Skull Hub] Character or HumanoidRootPart not found!")
            return
        end
